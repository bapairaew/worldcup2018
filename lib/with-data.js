...
export const getBalance = ({ data, bets = [], ignoreRemaining = false, ignoreMatch = x => false, now = moment() }) => {
  const initial = PER_DAY * now.diff(START_DAY, 'days')
  const balance = bets.reduce((remaining, bet) => {
    const m = findMatch(data, bet.match)
    if (m && !ignoreMatch(m)) {
       if (!m.finished) {
        return remaining - (ignoreRemaining ? 0 : bet.amount)
          ...
          <Query query={getPlayer} variables={{ slackid }}>
            {({ data: { Player = {} } = {}, ...props}) => (
              <DataContext.Provider value={{
                  ...this.state,
                  balance: data ? getBalance({ data, bets: Player.bets }) : 0,
