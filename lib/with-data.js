...
`, {
  ...
  constructor () {
    super()
    const user = process.browser ? JSON.parse(window.localStorage.getItem(STORAGE_KEY)) : undefined
    this.state = {
      user,
      ...
      <Query query={getPlayer} variables={{ slackid }}>
        {({ data: { Player = {} } = {}, ...props}) => (
          <DataContext.Provider value={{
            ...
            {!user && !initing && process.browser && <LoginModal />}
