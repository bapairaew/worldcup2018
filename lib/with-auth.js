...
export const withAuth = (Component) => graphql(slackToken, {
  props: ({ mutate }) => ({
    signInSignUp: (slackcode) =>
      mutate({
        variables: { slackcode }
      })
  })
  ...
  async componentDidMount () {
    if (!_user.name) {
      let slacktoken = window.localStorage.getItem(SLACK_STORAGE_KEY)
      if (!slacktoken) {
        const { code } = queryString.parse(window.location.search)
        if (code) {
          window.history.pushState(null, '', window.document.location.href.split('?')[0])
          const { data: { signInSignUp: { slackid, slacktoken, name, image } } } = await this.props.signInSignUp(code)
          console.log( { slackid, slacktoken, name, image })
          window.localStorage.setItem(SLACK_STORAGE_KEY, slacktoken)
          this.setState({ user: { id: slackid, name, image } })
        }
      } else {
        const { user: { id, name, image_192 } } = await (await fetch(`${SLACK_IDENTITY_ENDPOINT}?token=${slacktoken}`)).json()
        this.setState({ user: { id, name, image: image_192 } })
      }
    } else {
