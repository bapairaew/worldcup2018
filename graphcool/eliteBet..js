...
module.exports = async (event) => {
  try {
    ...
    const initial = Player.elite
    const balance = Player.eliteBets.reduce((remaining, bet) => {
    ...
    if (balance >= amount) {
      const { createEliteBet } = await api.request(`
        mutation {
          createEliteBet (
            userId: "${Player.id}"
            amount: ${amount}
      ...
      if (createEliteBet && createEliteBet.id) {
        return {
          data: {
            id: createEliteBet.id,
            amount,
            match,
            team
          }
